#List all the server directories you want to modify
serverDirectories = ['/home/minecraft/gtnh/ballers']
...
for file in os.listdir(os.getcwd()):
    if file.endswith('.zip') and file.__contains__("Server"):
...
if len(packArchives) == 0:
    ...
    for serverDirectory in serverDirectories:
        print("Updating server: " + serverDirectory.split("/")[-1])
        processed = 0
        serverModFolder =   os.listdir(os.path.join(serverDirectory, "mods"))
        existingMods = {}
        updateMods = []
        possibleNewMods = []
        missed = []
        for mod in serverModFolder:
            if not os.path.isfile(os.path.join(serverDirectory, "mods", mod)):
                folder = os.listdir(os.path.join(serverDirectory, "mods", mod))
                for modInFolder in folder:
                    if os.path.isfile(os.path.join(serverDirectory, "mods", mod, modInFolder)):
                        path, name, version =  getModName(modInFolder)
        ...
        for mod in  os.listdir(os.path.join(updateDirectory, "mods")):
            ...
            try:
                if compareVersions(version, existingMods[name][2]):
                    ...
                    processed += 1
                    updateMod(existingMods[name][0], path, os.path.join(updateDirectory, "mods"), os.path.join(serverDirectory, "mods"))
                else:
                    #Downgrading mods?
        ...
        for mod in possibleNewMods:
            ...
            if maxSimilarity > 0.95:
                processed += 1
                updateMod(existingMods[bestMatch][0], path, os.path.join(updateDirectory, "mods"), os.path.join(serverDirectory, "mods"))
            else:
                newMods.append(mod)
        ...
        for mod in newMods:
            ...
            try:
                ...
                print(mod.ljust(30))
                processed += 1
                addMod(mod, os.path.join(updateDirectory, "mods"), os.path.join(serverDirectory, "mods"))
        ...
        for mod in existingMods.keys():
            if mod.lower() not in updateMods:
                print(mod)
                try:
                    removeMod(existingMods[mod][0], os.path.join(serverDirectory, "mods"))
                except:
        ...
        #Update scripts
        try:
            replaceFolder(os.path.join(serverDirectory, "scripts"), os.path.join(updateDirectory, "scripts"))
        except:
            print("No scripts folder found.")
        #Update libraries
        try:
            replaceFolder(os.path.join(serverDirectory, "libraries"), os.path.join(updateDirectory, "libraries"))
        except:
            print("No libraries folder found.")
        #Update configs
        try:
            replaceFolder(os.path.join(serverDirectory, "config"), os.path.join(updateDirectory, "config"))
        except:
